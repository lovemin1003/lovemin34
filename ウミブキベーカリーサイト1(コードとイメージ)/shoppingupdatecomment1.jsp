...
ArrayList<bbs1> list2 = bbs1dao.getlist1(commentid);
bbsid=list2.get(0).getBbsid();
shopbbs1dao bbsdao1 = new shopbbs1dao();

ArrayList<bbs1> list = bbsdao1.getlist(bbsid);
...
<thead>
<tr>
<th colspan="3" style="background-color: pink; text-align: center;">購買後記　掲示板</th>
</tr>
</thead>
...
<a href="shoppingbbs.jsp" class="btn btn-primary">目録</a>
<%
	if(userid != null && userpassword!=null && userid.equals(list1.get(0).getUserid()) )
	{
%> 
<a onclick="return confirm('本当に修正しますか？')" 
href="shoppingupdate.jsp?bbsid=<%=bbsid%>" class="btn btn-primary" style="background-color:blue;">修正</a>
<a onclick="return confirm('本当に削除しますか？')"  
 href="shoppingdeleteaction.jsp?bbsid=<%=bbsid%>" class="btn btn-primary">削除</a>
...
<%= list.get(i).getUserid()  %>:<%= list.get(i).getBbscontent1()  %>
<tr>
<td style="background-color:black; color:white; ">添付 ファイル:</td>
...
<form method="post" action="shoppingupdatecommentaction.jsp?commentid=<%=commentid%>&bbsid=<%=bbsid%>"enctype="multipart/form-data">
...
<a  href="shoppingview.jsp?bbsid=<%=bbsid%>" class="btn btn-primary pull-right" style="background-color:blue;">戻る</a>

<input type="submit" onclick="return confirm('ファイルを指定しないままで修正すると　前に載せたファイルは無くなります。このままで修正しますか？')" class="btn btn-primary pull-right" value="コメント修正">
<% %>
</form>
