...
if(bbsid==0)
{
	...
	script.println("alert('そのような文はありません。')");
...
<%
shopbbs1dao bbs1dao1 = new shopbbs1dao();
...
%>
<h4 style="text-align: right;">閲覧数:<%=bbs.getHit()%> コメント数:<%=bbs1dao.commentcount(bbsid) %></h4>
<tbody>
<tr>
...
<source src="Storage/<%=bbs.getFilename() %>" type='video/ogg'>
</video>
...
%>
<a href="shoppingview.jsp?bbsid=<%=bbsid-1 %>" class="btn btn-primary">前の文</a>
<%
}
%>
<%
...
{
%>
<a href="shoppingview.jsp?bbsid=<%=bbsidup+1 %>" class="btn btn-primary">次の文</a>
<%
}
%>
...
href="shoppingupdate.jsp?bbsid=<%=bbsidud%>" class="btn btn-primary" style="background-color:blue;">修正</a>
<a onclick="return confirm('本当に　削除しますか？')"  
 href="shoppingdeleteaction.jsp?bbsid=<%=bbsidud%>" class="btn btn-primary">削除</a>
...
<a onclick="return confirm('本当に　修正しますか？')" 
href="shoppingupdatecomment1.jsp?commentid=<%=list.get(i).getCommentid()%>&bbsid<%=bbsid %>" class="btn btn" style="color:blue;">修正</a>
<a onclick="return confirm('진本当に　削除しますか？')"  
 href="shoppingdeletecommentaction.jsp?commentid=<%=list.get(i).getCommentid()%>" class="btn btn">削除</a>
...
<form method="post" action="shoppingcommentaction.jsp?commentid=<%=commentid%>&bbsid=<%=bbsid%>" enctype="multipart/form-data">
